# ui-structure-converter Specification

## Purpose

定义将 Image-To-UI `ui_structure.json` 转换为 Unity UGUI 节点描述符（UguiNodeDescriptor）的转换器能力，包括反序列化模型、锚点/布局映射、坐标系翻转、元素类型到组件映射和层级树递归转换。
