...
定义 RogueCard 项目 Unity Test Runner PlayMode 测试套件的组织约定、测试隔离机制，以及 EF 框架运行时基础设施（YooAsset / SceneManager / EntityManager / UniTask + Timer）的可观察验证契约。本 capability 不覆盖 UI Toolkit Navigator/Screen 与 SoundManager（剥离至后续变更）。
