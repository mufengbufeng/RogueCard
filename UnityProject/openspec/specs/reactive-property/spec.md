# reactive-property Specification

## Purpose

定义 ReactiveProperty 响应式属性和 ViewModelBase 基类，作为 MVVM UI 框架的数据绑定基础。ReactiveProperty 在值变化时触发 Changed 事件；ViewModelBase 通过 Prop<T> 工厂方法自动追踪所有响应式属性，Dispose 时一键清理所有监听者，避免事件泄漏。
