...
定义 EF.UI 框架下 Screen / ViewModel / UXML / USS 四件套的命名约定与 Popup 标记基类，使新增 Screen 完全脱离中心化注册——`{Stem}View` 类决定一切下游资源加载行为，Navigator 通过命名约定 + 泛型反射在打开时自动解析 ViewModel 类型与资源名。约定可通过 Screen 子类的虚属性 override 处理罕见特例。
