...
定义主菜单开始游戏后进入局内 UI 的流程。MainMenuProcedure 承接 UGUI MainView/MainController 发出的默认关卡进入请求，通过流程状态机切换到 GameProcedure；GameProcedure 创建 GameViewModel 与各 System，并通过 `IUIManager.OpenWindowAsync<GameView, GameController>()` 打开 UGUI GameView，订阅 UI 命令转发到 System。
## Requirements
