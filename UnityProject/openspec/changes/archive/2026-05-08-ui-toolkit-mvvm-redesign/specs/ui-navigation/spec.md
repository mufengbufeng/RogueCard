...
Navigator.PopPopup() SHALL 移除 PopupLayer 中最后添加的弹窗和对应的遮罩层，调用弹窗的 OnHide 和 OnDispose，并 Dispose ViewModel。
...
Screen<TViewModel> SHALL 继承 VisualElement。LoadContent(vta) SHALL 将 VTA 克隆内容作为子节点添加。Setup(viewModel) SHALL 存储 ViewModel 引用并调用 OnSetup()。OnSetup 为抽象方法，子类在此执行 UQuery 和数据绑定。
