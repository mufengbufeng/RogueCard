...
from mempalace.miner import detect_room, mine, scan_project, status
...
def test_scan_project_includes_rst_design_docs():
    tmpdir = tempfile.mkdtemp()
    try:
        project_root = Path(tmpdir).resolve()
        write_file(project_root / "docs" / "architecture.rst", "Architecture\n" * 20)

        assert scanned_files(project_root) == ["docs/architecture.rst"]
    finally:
        shutil.rmtree(tmpdir)
...
def test_detect_room_uses_multilingual_design_doc_aliases(tmp_path):
    project_root = tmp_path
    filepath = project_root / "docs" / "架构设计" / "payment-gateway.md"
    write_file(
        filepath,
        "# Payment Gateway Design\n\n我们决定采用事件驱动架构，并记录在 ADR 中。\n",
    )

    rooms = [
        {"name": "architecture", "description": "Architecture docs"},
        {"name": "general", "description": "General docs"},
    ]

    room = detect_room(filepath, filepath.read_text(encoding="utf-8"), rooms, project_root)
