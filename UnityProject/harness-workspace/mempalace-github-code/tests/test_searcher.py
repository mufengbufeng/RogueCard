...
class TestSearchMemories:
    ...
    def test_search_memories_reranks_mixed_language_design_doc(self):
        """Lexical reranking should rescue mixed Chinese/English design docs."""
        mock_col = MagicMock()
        mock_col.query.return_value = {
            "documents": [
                [
                    "General planning notes for Q3 roadmap and staffing.",
                    "Payment gateway technical design. 我们决定采用事件驱动架构处理支付 webhook。",
                ]
            ],
            "metadatas": [
                [
                    {"wing": "project", "room": "planning", "source_file": "roadmap.md"},
                    {
                        "wing": "project",
                        "room": "architecture",
                        "source_file": "payment_design.md",
                    },
                ]
            ],
            "distances": [[0.05, 0.45]],
        ...
        with patch("mempalace.searcher.get_collection", return_value=mock_col):
            result = search_memories("支付 gateway design", "/fake/path")

        assert result["results"][0]["source_file"] == "payment_design.md"
        assert result["results"][0]["similarity"] > 0.5

    def test_search_memories_can_fallback_to_lexical_scan_for_cjk_queries(self):
        """When vector search misses, bounded lexical scan should still find mixed docs."""
        mock_col = MagicMock()
        mock_col.query.return_value = {
            "documents": [[]],
            "metadatas": [[]],
        ...
        mock_col.get.side_effect = [
            {
                "documents": [
                    "支付 gateway 设计文档\n决策：采用幂等 webhook 处理器。",
                    "Unrelated changelog entry for release process.",
                ],
                "metadatas": [
                    {
                        "wing": "project",
                        "room": "architecture",
                        "source_file": "payment-adr.md",
                    },
                    {"wing": "project", "room": "general", "source_file": "CHANGELOG.md"},
                ],
                "ids": ["doc-1", "doc-2"],
            },
        ...
        with patch("mempalace.searcher.get_collection", return_value=mock_col):
            result = search_memories("支付 gateway 架构", "/fake/path")

        assert result["results"][0]["source_file"] == "payment-adr.md"
        assert result["results"][0]["retrieval"] in {"lexical", "hybrid"}
