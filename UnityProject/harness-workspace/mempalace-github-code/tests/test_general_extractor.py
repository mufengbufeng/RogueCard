...
def test_extract_memories_mixed_language_design_doc():
    text = (
        "# Payment Gateway Design\n\n"
        "## 背景\n现有支付链路在 webhook 重试时容易重复扣款。\n\n"
        "## 决策\n我们决定使用 PostgreSQL outbox pattern，而不是直接同步调用，"
        "because the rollback path is clearer and the design doc needs stronger idempotency.\n\n"
        "## 风险\n如果消息积压，需要准备回滚方案。"
    )
    result = extract_memories(text)
    assert len(result) >= 1
...
def test_split_into_segments_by_design_doc_headings():
    text = (
        "# Design Doc\n"
        "Background\n"
        "Current system times out.\n"
        "Decision\n"
        "Use a queue.\n"
        "风险\n"
        "Need rollback.\n"
...
def test_is_section_heading_detects_design_doc_sections():
    assert _is_section_heading("## Decision")
    assert _is_section_heading("风险")
    assert _is_section_heading("背景：")
