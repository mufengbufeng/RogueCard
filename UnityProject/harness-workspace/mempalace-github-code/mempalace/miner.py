...
def detect_room(filepath: Path, content: str, rooms: list, project_path: Path) -> str:
    ...
    def room_candidates(room: dict) -> list:
        """Expand configured room names with common multilingual aliases."""
        name = room["name"]
        normalized_name = _SEPARATOR_RE.sub(" ", name.lower())
        candidates = [name.lower()]
        candidates.extend(k.lower() for k in room.get("keywords", []))
        for alias_root, aliases in ROOM_ALIAS_KEYWORDS.items():
            if alias_root in normalized_name:
    ...
    for part in path_parts[:-1]:  # skip filename itself
        for room in rooms:
            candidates = room_candidates(room)
    ...
    for room in rooms:
        candidates = room_candidates(room)
        if any(candidate in filename or filename in candidate for candidate in candidates):
            return room["name"]

    # Priority 3: keyword scoring from room keywords + name
    scores = defaultdict(int)
    for room in rooms:
        keywords = room_candidates(room)
        for kw in keywords:
