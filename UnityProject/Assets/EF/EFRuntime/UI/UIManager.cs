...
namespace EF.UI
{
    ...
    public sealed class UIManager : AEFManager, IUIManager
    {
        ...
        /// <summary>
        /// 为 EditMode 测试替换 Prefab 加载器，避免直接构造 YooAsset 句柄。
        /// </summary>
        internal void SetPrefabLoaderForTests(IUIManagerPrefabLoader prefabLoader)
        {
            _prefabLoader = prefabLoader ?? new ResourcePrefabLoader(_resourceManager);
        }
        ...
        private async UniTask<UIWindowInstance> CreateOrReuseInstanceAsync(UIWindowDescriptor descriptor, object userData, CancellationToken cancellationToken)
        {
            ...
            if (instance != null)
            {
                Transform layerRoot = ResolveRequiredLayerRoot(descriptor.Layer);
            ...
            UIPrefabLoadResult loadResult = default;
            ...
            try
            {
                ...
                loadResult = await _prefabLoader.LoadAsync(descriptor.Location, cancellationToken);
                hasLoadResult = true;
                cancellationToken.ThrowIfCancellationRequested();

                GameObject prefab = loadResult.Prefab;
                if (prefab == null)
                {
                ...
                instance.Attach(view, controller, loadResult, context);
            ...
            catch
            {
                if (hasLoadResult)
                {
                    _prefabLoader.Release(loadResult);
                }
        ...
        private void DisposeInstance(UIWindowInstance instance)
        {
            if (instance.View != null)
            {
                ...
                DestroyObject(instance.View.gameObject);
            }
            ...
            if (instance.HasLoadResult)
            {
                _prefabLoader.Release(instance.LoadResult);
            }
        ...
        private Transform ResolveRequiredLayerRoot(UILayer layer)
        {
            ...
            throw new InvalidOperationException($"UI 层级 {layer} 未注册，且没有设置 fallback root");
        }
        ...
        private sealed class UIWindowInstance
        {
            ...
            public UIPrefabLoadResult LoadResult { get; private set; }

            public bool HasLoadResult { get; private set; }

            public UIRuntimeContext Context { get; private set; }

            public UIWindowState State { get; set; }

            public void Attach(UIView view, UIController controller, UIPrefabLoadResult loadResult, UIRuntimeContext context)
            {
                View = view;
        ...
        private sealed class ResourcePrefabLoader : IUIManagerPrefabLoader
        {
            private readonly IResourceManager _resourceManager;

            public ResourcePrefabLoader(IResourceManager resourceManager)
            {
                _resourceManager = resourceManager ?? throw new ArgumentNullException(nameof(resourceManager));
            }

            public async UniTask<UIPrefabLoadResult> LoadAsync(string location, CancellationToken cancellationToken)
            {
                var handle = await _resourceManager.LoadAssetAsync<GameObject>(location, null, 0);
                cancellationToken.ThrowIfCancellationRequested();
                return new UIPrefabLoadResult(handle.AssetObject as GameObject, handle);
            }
            ...
            public void Release(UIPrefabLoadResult result)
            {
                ...
                {
                    _resourceManager.Release(result.Handle);
                }
            }
        }
    }
